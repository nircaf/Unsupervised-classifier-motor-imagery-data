...
%               1 	No AutoCalib
%               2	Concatanated & autocalibration (channels with lower sampling rate if more than 1 record is loaded)
%               3	Concatanated & no autocalibration (channels with lower sampling rate if more than 1 record is loaded)
%
% Author: Alois Schloegl, 03.02.1998, updated T.S. Lorig Sept 6, 2002 for BDF read
%         updated comments (Julien Kronegg, 19.02.2003)
%
% See also: openbdf()
...
%	V=sum([(S'==EDF.DigMax(:,ones(RecLen,1))) + (S'==EDF.DigMin(:,ones(RecLen,1)))])==0;
	...
	if floor(Mode/2)==1
        % Concatanated Mode (channels with lower sampling rate if more than 1 record is loaded)
		for k=1:EDF.NS,
