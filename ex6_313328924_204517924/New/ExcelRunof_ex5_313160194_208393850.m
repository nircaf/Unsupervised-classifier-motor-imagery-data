...
for run = 1: 50
    ...
    testDataName = 'motor_imagery_test_data.mat';%test set
    ...
    %band power:
    feat.bandPower{1} = {[12,15],[3.5,6]};   %Low Beta
    feat.bandPowerName{1} = "Low Beta 1";
    feat.bandPower{2} = {[15,19],[3.5,6]};   %Low Beta
    feat.bandPowerName{2} = "Low Beta 2";
    feat.bandPower{3} = {[30,34],[4,6]};     %Gamma
    feat.bandPowerName{3} = "Gamma";
    feat.bandPower{4} = {[8,12],[5.5,6]};    %Alpha
    feat.bandPowerName{4} = "Alpha";
    feat.bandPower{5} = {[19,23],[1.2,2.7]}; %High Beta
    feat.bandPowerName{5} = "High Beta 1";
    feat.bandPower{6} = {[23,30],[1.2,2.7]}; %High Beta
    feat.bandPowerName{6} = "High Beta 2";
    ...
    %perform spectrogram
    for i =1:length(Data.CaseName)    %over all conditions
    ...
    for featDeduct=1: feat.selectedFeat
    ...
    %% Test
    load(testDataName) %Load test set
    ...
    run = run +1;
    save('run.mat','run');
    xlswrite('My_file.xls',mean(accuracy),'Sheet1',sprintf('A%g',run));     %Write data
        xlswrite('My_file.xls',(feat.selectedFeat-bestFeatDeduct+1),'Sheet1',sprintf('B%g',run));     %Write data
